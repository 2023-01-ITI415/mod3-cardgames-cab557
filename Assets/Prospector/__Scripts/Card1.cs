...
namespace Poker
{
    public class Card1 : MonoBehaviour
    {
        ...
        public void Init(char eSuit, int eRank, bool startFaceUp = true)
        {
            ...
            def = JsonParseDeck1.GET_CARD_DEF(rank);
        ...
        {
            // Add Decorators
            foreach (JsonPip pip in JsonParseDeck1.DECORATORS)
            {                         // e
                if (pip.type == "suit")
                {
                    // Instantiate a Sprite GameObject
                    _tGO = Instantiate<GameObject>(Deck1.SPRITE_PREFAB, transform);       // f
        ...
        private void AddPips()
        {
            ...
            {                                   // b
                ...
                _tGO = Instantiate<GameObject>(Deck1.SPRITE_PREFAB, transform);
        ...
        private void AddFace()
        {
            ...
            _tGO = Instantiate<GameObject>(Deck1.SPRITE_PREFAB, transform);     // d
            _tSRend = _tGO.GetComponent<SpriteRenderer>();
